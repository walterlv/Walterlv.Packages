...
namespace Walterlv.Logging.Markdown
{
    ...
    public sealed class MarkdownLogger : TextFileLogger
    {
        ...
        /// <param name="shopuldAppendInfo">如果你希望每次创建同文件的新实例时追加到原来日志的末尾，则设为 true；如果希望覆盖之前的日志，则设为 false。</param>
        ...
        public MarkdownLogger(FileInfo infoLogFile, FileInfo errorLogFile,
            bool shopuldAppendInfo = false, bool shouldAppendError = false, string lineEnd = "\n")
            : base(infoLogFile, errorLogFile, shopuldAppendInfo, shouldAppendError, lineEnd)
        {
        }
