...
namespace Walterlv.Logging.Core
{
    ...
    public readonly struct LogContext
    {
        ...
        internal LogContext(DateTimeOffset time, string callerMemberName, string text, string? extraInfo, LogLevel currentLevel)
        {
            Time = time;
        ...
        {
            return obj is LogContext context &&
        ...
        /// 判断两条日志是否表示同一个日志。
        /// </summary>
        public static bool operator ==(LogContext left, LogContext right) => left.Equals(right);

        /// <summary>
        /// 判断两条日志是否表示不同日志。
        /// </summary>
        public static bool operator !=(LogContext left, LogContext right) => !(left == right);
    }
}
