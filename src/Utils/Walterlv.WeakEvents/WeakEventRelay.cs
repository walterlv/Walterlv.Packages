...
public abstract class WeakEventRelay<TEventSource> where TEventSource : class
{
    ...
    /// </summary>
    /// <param name="sourceEventAdder">请始终写为 <c>o => o.事件名 += On事件名</c>；例如 <c>o => o.Changed += OnChanged</c>。</param>
    /// <param name="relayEventAdder">请始终写为 <c>() => 弱事件.Add(value, value.Invoke)</c>；例如 <c>() => _changed.Add(value, value.Invoke)</c>。</param>
    ...
    /// <summary>
    /// 请在原始事件的事件处理函数中调用此方法，并且请始终写为 <c>TryInvoke(弱事件, sender, e)</c>。
    /// </summary>
